...
export async function GET(req: NextRequest, { params }: any) {
  try {
    ...
    const swaps = await moralis.token.getTokenSwapsByTokenAddress(address, {
    ...
    if (!swaps) throw BadRequestError("Could not get token swaps");

    return NextResponse.json(
      {
        success: true,
        message: "Token swaps fetched!",
        data: swaps,
      },
      { status: 200 }
    );
